...
# 428 "./mcc_generated_files/pin_manager.h"
...
# 78 "./mcc_generated_files/eusart.h"
...
# 161 "./mcc_generated_files/eusart.h"
...
# 300 "./mcc_generated_files/eusart.h"
...
void opll_write(uint8_t addr, uint8_t val)
{
    ...
    do { LATCbits.LATC2 = 1; } while(0);
    do { LATCbits.LATC5 = 1; } while(0);
    _delay((unsigned long)((3)*(16000000/4000000.0)));
...
uint8_t i, ch, oct;
    ...
    if(vel) {
        ...
        opll_write(0x20 + ch, 0);
        opll_write(0x30 + ch, (synth.instrument << 4) | (~vel & 0xf));
        opll_write(0x10 + ch, (uint8_t)n);
        opll_write(0x20 + ch, 0x10 | oct);
...
# 255 "main.c"
static volatile char dpy_framebuf[32] = {
    ' ', ' ', ' ', ' ', 'L', 'a', 'z', 'y', ' ', '\'', '8', '8', ' ', ' ', ' ', ' ',
...
void dpy_reset(void)
{
    ...
    _delay((unsigned long)((37)*(16000000/4000000.0)));
...
uint8_t i;
    ...
    if((((ctl_status & (CTL_ENCODER_A)) != (ctl_status_prev & (CTL_ENCODER_A))) || ((ctl_status & (CTL_ENCODER_B)) != (ctl_status_prev & (CTL_ENCODER_B)))) && (ctl_status & 0x3) == 0) {
    ...
    if(ctl_encoder_dir) {
        ...
        else if((ctl_encoder_dir > 0 && synth_param_cur < (10 - 1)) || (ctl_encoder_dir < 0 && synth_param_cur > 0)) {
            synth_param_cur += ctl_encoder_dir;
            dpy_need_update = 1;
        }
    }

    if((((ctl_status & (CTL_ENCODER_BUTTON)) != (ctl_status_prev & (CTL_ENCODER_BUTTON))) && (!(ctl_status & (CTL_ENCODER_BUTTON))))) {
        synth_param_editing = !synth_param_editing;
        dpy_need_update = 1;
    }

    if((((ctl_status & (CTL_BUTTON1)) != (ctl_status_prev & (CTL_BUTTON1))) && (!(ctl_status & (CTL_BUTTON1))))) {
        button_press_ms[0] = 1;
    }
    else if((((ctl_status & (CTL_BUTTON1)) != (ctl_status_prev & (CTL_BUTTON1))) && (ctl_status & (CTL_BUTTON1)))) {
        button_press_ms[0] = 0;
    }
}
# 610 "main.c"
...
uint8_t d, cur_col, midi_data[8];
    ...
    while(1) {
        if(dpy_need_update) {
            cur_col = synth_param_snprint((char*)dpy_framebuf, 32 / 2, synth_param_cur);
            if(!synth_param_editing)
                cur_col = 0;

            if(synth_param_cur < (10 - 1))
                synth_param_snprint((char*)dpy_framebuf + 32 / 2, 32 / 2, synth_param_cur + 1);
            else
                memset((void*)(dpy_framebuf + 32 / 2), ' ', 32 / 2);
